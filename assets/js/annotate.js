(function () {
  ...
  async function loadQuestions(){
    ...
    let arr=all.filter(x=>norm(x.Category)===norm(requested));
    if(arr.length===0 && requested){
      arr=all.filter(x=>norm(x.Category).includes(norm(requested)));
    }
    if(arr.length===0) arr=all.slice();
  ...
  let evaluated=[], evaluatedOutcome=[];
  ...
  function captureCurrent(){
    ...
    // التقط الاختيار (ندعم name=choice أو name=mcqOption)
  ...
  if(btnNext) btnNext.addEventListener("click", ()=>{
    ...
    if(res===null) return; // لسه ما اختيرت إجابة
    if(AUTO_ADVANCE_DELAY_MS>0){ setTimeout(()=>{ hidePerQuestionFeedback(); goNext(); }, AUTO_ADVANCE_DELAY_MS); }
    else { hidePerQuestionFeedback(); goNext(); }
  });
  if(btnDone) btnDone.addEventListener("click", ()=>{
    ...
    const store=JSON.parse(localStorage.getItem(key)||'{"sessions":[]}');
