 ...
 * مخزن النتائج:
 * key = thq_results_v1:<username>
 * value = { sessions: [ { ts, projectId, type, category, source, total, gradedTotal, correct,
...
(function () {
  ...
  async function loadQuestions() {
    ...
    // دوال التعرف على النوع
    function isMCQ(x) {
      return (
        String(x.type || x.Type || "").toUpperCase().includes("MCQ") ||
  ...
  function captureCurrent() {
    ...
    if (!rec) {
      rec = { qid, type, picked: null, text: "", correct: null, isGraded: false };
